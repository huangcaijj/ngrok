...
install_go(){
	...
	if [ $(getconf WORD_BIT) = '32' ] && [ $(getconf LONG_BIT) = '64' ];then
		...
		if [ ! -f $SELFPATH/go1.7.6.linux-amd64.tar.gz ];then
			wget https://golang.org/doc/install?download=go1.12.5.linux-amd64.tar.gzz --no-check-certificate
		fi
	    tar zxvf go1.7.6.linux-amd64.tar.gz
	else
		if [ ! -f $SELFPATH/go1.7.6.linux-386.tar.gz ];then
			wget https://golang.org/doc/install?download=go1.12.5.linux-386.tar.gz --no-check-certificate
		fi
